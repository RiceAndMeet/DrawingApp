...
c.addEventListener("mousedown",function(event){
	...
	if (selected == "image" && image != null && !image.contains(initial_x,initial_y))
		dragged = false ;
	
	else if (selected == "image" && image != null){
		if (image.c2.contains(initial_x,initial_y)){
			resize= true;
			dragged = true;
		}
		else if (image.c5.contains(initial_x,initial_y)){
			resize= true;
			dragged = true;
		}
...
c.addEventListener("mousemove",function(event){
	...
	if (dragged){
		...
		if (selected == "image"){
			...
			if (resize==true){
				image=new CanvasImage(image.c1.x,image.c1.y,x-image.c1.x,y-image.c1.y,image.src);
			}
			else image=new CanvasImage(x-width/2,y-length/2,width,length,image.src);
		}	
		repaint();
	}
